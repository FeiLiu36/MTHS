...
class TSPSolver:
    ...
    def _guide(self,distance_matrix: np.ndarray, num_clusters: int = 3) -> np.ndarray:
        num_cities = distance_matrix.shape[0]
        heuristics_matrix = np.zeros_like(distance_matrix, dtype=float)
        penalties = np.zeros_like(distance_matrix, dtype=float)
        ...
        # Clustering to identify local proximity
        kmeans = KMeans(n_clusters=min(num_cities, num_clusters), random_state=0)
        ...
        for i in range(num_cities):
            for j in range(num_cities):
                if i != j:
                    contrib = distance_matrix[i, j] - np.min(distance_matrix[i, :]) - np.min(distance_matrix[:, j])
                    contrib = max(contrib, 0)

                    # Update penalties dynamically with historical context
                    penalties[i, j] = (1 - learning_rate) * penalties[i, j] + learning_rate * contrib

                    # Compute heuristics with consideration of penalties
                    heuristics_matrix[i, j] = contrib + penalties[i, j]

                    # Enhance heuristics for edges within the same cluster
                    if clusters[i] == clusters[j]:
                        heuristics_matrix[i, j] *= cluster_weight

                    # Encourage diversity for used edges
                    if contrib > 0:
                        heuristics_matrix[i, j] *= diversity_factor

            # Adaptive learning rate based on heuristics performance
            learning_rate = min(0.5, learning_rate * (1 + np.mean(heuristics_matrix[i, :])))

            # Decay penalties
            penalties[i, :] *= decay_factor

        # Normalize heuristics matrix
        max_value = np.max(heuristics_matrix, initial=1)  # Avoid division by zero
